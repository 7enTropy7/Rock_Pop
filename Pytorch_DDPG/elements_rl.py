...
class Environment():
    ...
    def step(self,action,shoot):
        if action[0]<=0.5:
            self.player.triangle_body.velocity = -150*(0.5-action[0]),0
        elif action[0]>0.5:
            self.player.triangle_body.velocity = 150*(action[0]),0
        ...
        if self.player.collided == False:
            if shoot < 500:
                ...
                self.reward = -0.00002*shoot
            
        if self.rock.hit_by_bullet == True:
            self.reward = 3
            self.rock.hit_by_bullet = False
        done = self.done
...
class Rock():
    def __init__(self,mass,radius,space,screen):
        ...
        self.circle_body.position = random.randint(10,900),30
        self.circle_shape = pymunk.Circle(self.circle_body,self.radius)
        self.circle_shape.body.velocity = (random.choice([100,-100]),0)
...
class Player():
    ...
    def draw_player(self):
        if self.triangle_body.position[0] >= 950:
             self.triangle_body.position = 950,720
        elif self.triangle_body.position[0] <= 50:
             self.triangle_body.position = 50,720
